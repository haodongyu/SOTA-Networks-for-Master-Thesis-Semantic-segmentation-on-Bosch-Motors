...
class MotorDataset(Dataset):
    def __init__(self, split='train', data_root='trainval_fullarea', num_point=4096, test_area=1, block_size=1.0, sample_rate=1.0, transform=None):
        super().__init__()
        self.num_point = num_point
        self.block_size = block_size
        self.transform = transform
        motors = sorted(os.listdir(data_root))
        motors = [motor for motor in motors if 'Scan_' in motor]
        if split == 'train':
            motors_split = [motor for motor in motors if not 'Scan_{}'.format(test_area) in motor]
        else:
            motors_split = [motor for motor in motors if 'Scan_{}'.format(test_area) in motor]
