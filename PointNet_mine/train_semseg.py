...
writer = SummaryWriter(BASE_DIR + '/tensor')
   ...
   # classifier = MODEL.get_model(NUM_CLASSES).cuda()
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        with torch.no_grad():
            ...
            noBG_mIoU = np.mean(np.array(noBG_correct_class) / (np.array(noBG_iou_deno_class, dtype=np.float) + 1e-6))
            ...
            log_string('eval point avg class acc without background: %f' % (
                np.mean(np.array(noBG_correct_class) / (np.array(noBG_seen_class, dtype=np.float) + 1e-6))))
