...
def main(args):
   ...
   # os.environ["CUDA_VISIBLE_DEVICES"] = args.gpu
    ...
    TEST_DATASET_WHOLE_SCENE = ScannetDatasetGridMotor(root, split='test', test_area=args.test_area, block_points=NUM_POINT)
    ...
    if args.test_area == 'Validation':
            ...
            for batch_idx in range(num_batches):
                ...
                for _ in tqdm(range(args.num_votes), total=args.num_votes):
                    ...
                    for sbatch in range(s_batch_num):
                        ...
                        batch_pred_label = pred.detach().cpu().numpy()
        ...
        with torch.no_grad():
            ...
            for batch_idx in range(num_batches):
                ...
                total_iou_deno_class_tmp = [0 for _ in range(NUM_CLASSES)]
                if args.visual:
                    fout = open(os.path.join(visual_dir, scene_id[batch_idx] + '_pred_edit.obj'), 'w')
                   # fout_gt = open(os.path.join(visual_dir, scene_id[batch_idx] + '_gt.obj'), 'w')
